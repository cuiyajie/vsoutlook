...
type AudioMappingForm = Pick<AudioMapping, 'name' | 'mute'>
...
const audioCopys = ref<[number, number][]>([])

function addAudioCopy() {
  audioCopys.value.unshift([1, 1])
}
...
const zodSchema = z.object({
  ...
  mute: z.string({ required_error: "请输入静音声道范围" })
    .nonempty("请输入静音声道范围")
    .trim()
    .refine(value => {
...
useListener(Signal.OpenNewAudioMapping, (payload: {
  _callback?: any,
  index?: number,
}) => {
  opened.value = true
  ...
  if (mapping) {
    ...
    audioCopys.value = mapping.copys
  } else {
    form.value = {
      name: '',
      mute: ''
            ...
            <div v-for="(ac, idx) in audioCopys" :key="idx" class="columns is-multiline">
              ...
              <div class="column is-2">
                <VField class="copy-action">
                  <VControl>
                    <VIconButton color="danger" light raised circle icon="feather:x" @click="removeAudioCopy(idx)" />
                  </VControl>
                </VField>
              </div>
            </div>
            <div v-if="audioCopys.length === 0" class="form-empty">暂时没有添加音频映射模版</div>
          </div>
        </div>
      </div>
    </template>
