...
import {
  def_endswitch_input_params,
  def_endswitch_output_params,
  def_endswitch_input_video_params,
...
import { handle, unwrap, watchInput, watchNmosName, wrap } from '../Utilties/Utils'
      ...
      inputs.value = inputs.value.concat(
        Array.from({ length: nv - len }, (_, i) => {
          return {
            ...
            value: params[len + i]
              ? ref(params[len + i])
              : ref(def_endswitch_input_video_params(len + i)),
          }
        })
      )
    }
      ...
      outputs.value = outputs.value.concat(
        Array.from({ length: nv - len }, (_, i) => {
          return {
            ...
            value: params[len + i]
              ? ref(params[len + i])
              : ref(def_endswitch_output_params()),
          }
        })
      )
    }
...
function getValue() {
  ...
  return {
    ...
    output: {
      ...
      out_params: outputs.value.map((opt, idx) =>
        Object.assign(wrap(opt.value, 'out_', useb, false, mip, bip), { index: idx })
      ),
    },
  }
}
