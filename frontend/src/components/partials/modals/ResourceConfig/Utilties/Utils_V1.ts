...
export function handlePlayerParams<T extends PlayerParams>(params: T, vfs: VideoFormat[]) {
  const { videoformat_name, audioformat_name, smpte_params, stream_params, ...rest } = params
...
export function wrap(src: any, prefix: string) {
  ...
  for (const key in src) {
    let wrapKey = key;
    if (['v_src_address', 'v_dst_address', 'a_src_address', 'a_dst_address'].includes(key)) {
      wrapKey = `${prefix}${key}`
    }
    if (src[key] instanceof Array) {
      dst[wrapKey] = src[key].map((v: any, index: number) => Object.assign(wrap(v, prefix), { index }));
