  ...
  watch(
    ...
    (nv) => {
      ...
      if (mv.value?.videoformat) {
        ...
        mv.value = mv.value || {}
        mv.value.videoformat = omit(
          mv.value.videoformat,
          'v_width',
          'v_height',
          'v_interlaced',
          'v_fps',
...
export function getFormat(mv: any) {
  ...
  return `${vf.v_width}_${vf.v_height}_${Number(vf.v_interlaced)}_${vf.v_fps}_${
    vf.v_gamma
  }_${vf.v_gamut}`
}
  ...
  watch(
    ...
    ([df, w]) => {
      if (df === v_compression_format) {
        mv.value.videoformat.v_compression_ratio =
          String(w) === v_width[0] ? v_compression_ratio[0] : v_compression_ratio[1]
      } else {
    ...
    if (src[key] instanceof Array) {
      ...
      if (isBackup && !useBackup) {
        ...
        dst[wrapKey] = src[key].map((v: any, index: number) =>
          Object.assign(wrap(v, prefix, useBackup, isBackup, m_local_ip, b_local_ip), {
            index,
          })
        )
      }
