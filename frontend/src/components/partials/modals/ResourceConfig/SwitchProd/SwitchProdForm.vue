...
function getValue() {
  const result = {
    ...
    bus: handleSwitchBus(bus.value, mv.value.used_signal_type, 'makeswt'),
    output: wrap(handleSwitchOut(out.value, videoFormats.value, mv.value.audio_workmode, mv.value.used_signal_type, 'makeswt'), 'out_'),
  }
