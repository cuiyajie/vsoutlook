...
import mvData from '@src/data/vscomponent/mv_v1.json'
...
function getValue() {
  ...
  return {
    ...
    input: {
      ...
      input_params: inputs.value.map((ipt, index) =>
  ...
  nextTick(() => {
    outputs.value.forEach((optv, idx) => {
      ...
      if (
        ...
        !mvTemplateIsValid(optv.value.value.mv_template)
      ) {
        optv.value.value.mv_template = ''
      }
    })
