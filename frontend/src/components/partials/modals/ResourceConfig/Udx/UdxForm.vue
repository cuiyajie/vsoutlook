...
watch(
  ...
  (nv, ov) => {
    if (mv.value.mode === val_udx[0]) {
      inputFormat.value =
        inputFormat.value !== formatKeys[2] ? inputFormat.value : formatKeys[0]
      output1Format.value = formatKeys[2]
      output2Format.value = formatKeys[1]
    } else {
      inputFormat.value = formatKeys[2]
      output1Format.value =
        output1Format.value !== formatKeys[2] ? output1Format.value : formatKeys[1]
      output2Format.value =
        output2Format.value !== formatKeys[2] ? output2Format.value : formatKeys[1]
    }
...
function getValue() {
  ...
  return {
    ...
    output: {
      ...
      out_params: outputs.value
        .map((o, i) =>
          Object.assign(wrap(unref(o), 'out_', useb, false, mip, bip), { index: i })
        )
