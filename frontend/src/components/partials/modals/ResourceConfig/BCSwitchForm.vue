...
import { def_bcswitch_input_params, def_bcswitch_output_params, def_bcswitch_input_video_params, type BCSwitchInputParamsType, global_config, def_bcswitch_bus_params, type BCSwitchBusParamsType, type NMosConfigType, type SSMAddressType, nmos_config, ssm_address, type SwitchKeyType, def_bcswitch_bus_keyfill_params, def_switch_key_params, def_switch_key } from './Consts';
...
mv.value = pick(bcSwitchData, ['moudle', 'input_number', 'tallyserver_url', 'hw_panel_url', 'physic_nic_port0_ip', 'physic_nic_port1_ip', '2110-7_m_local_ip', '2110-7_b_local_ip', 'nmos', 'ssm_address_range', 'authorization_service', 'api_server_port'])
  ...
  if (nv < len) {
    ...
    inputs.value = inputs.value.concat(
      Array.from({ length: nv - len }, (_, i) => {
        return {
          index: len + i + 1,
          value: params[len + i] ? ref(params[len + i]) : ref(def_bcswitch_input_video_params(len + i))
        }
      })
    );
  }
  ...
  if (nv < len) {
    ...
    masterBusKeyFills.value = masterBusKeyFills.value.concat(
      Array.from({ length: nv - len }, (_, i) => {
        return {
          index: len + i + 1,
          value: kbmParams[len + i] ? ref(kbmParams[len + i]) : ref(def_bcswitch_bus_keyfill_params())
        }
      })
    );
    backupBusKeyFills.value = backupBusKeyFills.value.concat(
      Array.from({ length: nv - len }, (_, i) => {
        return {
          index: len + i + 1,
          value: kbbParams[len + i] ? ref(kbbParams[len + i]) : ref(def_bcswitch_bus_keyfill_params())
        }
      })
    );
  }
...
function setValue(data: typeof bcSwitchData) {
  mv.value = pick(data, ['moudle', 'input_number', 'tallyserver_url', 'hw_panel_url', 'physic_nic_port0_ip', 'physic_nic_port1_ip', '2110-7_m_local_ip', '2110-7_b_local_ip', 'nmos', 'api_server_port', 'ssm_address_range', 'authorization_service'])
