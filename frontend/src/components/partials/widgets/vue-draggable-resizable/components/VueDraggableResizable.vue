...
export default {
  ...
  computed: {
    style () {
      return {
        transform: `translate(${this.left}px, ${this.top}px) rotate(var(--vdr-rotate))`,
    ...
    handleResize (e) {
      ...
      if (this.handle.includes('b')) {
        ...
        if (this.centeredScaling) {
          ...
          const delta = Math.min(bottom - this.bottom, top - this.top)
          top = this.top + delta
        ...
        if (this.lockAspectRatio && this.resizingOnY) {
          ...
          if (this.centeredScaling) {
            left = this.left - (this.right - right)
            const delta = Math.min(right - this.right, left - this.left)
        ...
        if (this.centeredScaling) {
          bottom = restrictToBounds(
            this.bottom - (this.top - top),
            this.bounds.minBottom,
            this.bounds.maxBottom
          )
          const delta = Math.min(top - this.top, bottom - this.bottom)
          top = this.top + delta
          bottom = this.bottom + delta
        }
        ...
        if (this.lockAspectRatio && this.resizingOnY) {
          ...
          if (this.centeredScaling) {
            right = this.right - (this.left - left)
            const delta = Math.min(left - this.left, right - this.right)
            left = this.left + delta
            right = this.right + delta
          }
        }
      ...
      if (this.handle.includes('r')) {
        ...
        if (this.lockAspectRatio && this.resizingOnX) {
          ...
          if (this.centeredScaling) {
            top = this.top - (this.bottom - bottom)
            const delta = Math.min(bottom - this.bottom, top - this.top)
            top = this.top + delta
            bottom = this.bottom + delta
          }
        ...
        if (this.lockAspectRatio && this.resizingOnX) {
          ...
          if (this.centeredScaling) {
            bottom = this.bottom - (this.top - top)
            const delta = Math.min(top - this.top, bottom - this.bottom)
            top = this.top + delta
            bottom = this.bottom + delta
          }
