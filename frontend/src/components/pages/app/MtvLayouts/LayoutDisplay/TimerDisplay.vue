...
const fontFamily = computed(() => {
  ...
  return t.clockDisplayType === CLockDisplayType.Digital ? (t.fontFamily || defaultFontFamily) : 'Digital Dismay'
})

const fontFormat = computed(() => {
  const t = props.data?.timer
  if (!t) return "yyyy-MM-dd"
  return t.clockDisplayType === CLockDisplayType.Digital ? (t.dateDisplayType === DateDisplayType.Chinese ? "yyyy年MM月dd日" : "yyyy-MM-dd") : "yyyy-MM-dd"
})
...
<template>
  <div v-if="data?.timer && dateText" data-role="timer" class="layout-timer" :style="{fontSize: `${data.timer.fontSize * bound.w}px`, color, fontFamily}" style="max-width: 100%;">
    <slot />
