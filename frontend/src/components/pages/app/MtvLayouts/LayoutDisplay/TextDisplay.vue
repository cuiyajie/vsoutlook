...
<template>
  <div v-if="data?.text" data-role="text" class="layout-text" style="max-width: 100%;" :style="{gap: `${data.text.gap * bound.w}px`}">
    <slot />
    <div class="text-rect" :style="{ fontFamily: font1, color: color1, backgroundColor: bgColor1, fontSize: `${fontSize1 * bound.w}px` }">文字窗口 1</div>
    <div class="text-rect" :style="{ fontFamily: font2, color: color2, backgroundColor: bgColor2, fontSize: `${fontSize2 * bound.w}px` }">文字窗口 2</div>
  </div>
</template>
<style lang="scss">
</style>
