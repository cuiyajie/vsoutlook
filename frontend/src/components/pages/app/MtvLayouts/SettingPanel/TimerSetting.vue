...
const tx = computed({
  get: () => props.modelValue?.timer ? roundToNearestEven(props.modelValue.timer.x * props.base.w) : 0,
  set: (v: number) => {
...
const ty = computed({
  get: () => props.modelValue?.timer ? roundToNearestEven(props.modelValue.timer.y * props.base.h) : 0,
  set: (v: number) => {
    if (props.modelValue?.timer) {
...
const clockDisplayType = computed({
  get: () => props.modelValue?.timer?.clockDisplayType ? CLockDisplayType.LED : CLockDisplayType.Digital,
  set: (v: number) => {
    if (props.modelValue?.timer) {
      const related = v === CLockDisplayType.LED ? { dateDisplayType: DateDisplayType.Hyphen } : {}
      emit('update:model-value', { ...props.modelValue, timer: { ...props.modelValue.timer, clockDisplayType: v, ...related } })
    }
  }
})

const datetype = computed<DateDisplayType>({
  get: () => props.modelValue?.timer?.dateDisplayType ? DateDisplayType.Hyphen : DateDisplayType.Chinese,
  set: (v: number) => {
    if (props.modelValue?.timer) {
...
const timeX = computed({
  get: () => props.modelValue?.timer?.timePosition ? roundToNearestEven(props.modelValue.timer.timePosition.x * props.base.w) : 0,
  set: (v: number) => {
    if (props.modelValue?.timer) {
...
const timeY = computed({
  get: () => props.modelValue?.timer?.timePosition ? roundToNearestEven(props.modelValue.timer.timePosition.y * props.base.h) : 0,
  set: (v: number) => {
    if (props.modelValue?.timer) {
      const timePos = props.modelValue.timer.timePosition || { x: 0, y: 0 }
      emit('update:model-value', {
...
const fh = computed({
  get: () => props.modelValue?.timer ? Math.round(props.modelValue.timer.fontHeight * props.base.w) : 0,
  set: (v: number) => {
    if (props.modelValue?.timer) {
      const realV = clamp(v, 0, Math.floor(props.base.w / 5))
      emit('update:model-value', { ...props.modelValue, timer: { ...props.modelValue.timer, fontHeight: realV / props.base.w } })
    }
  }
})

const fg = computed({
  get: () => props.modelValue?.timer ? Math.round(props.modelValue.timer.fontGap * props.base.w) : 0,
  set: (v: number) => {
    if (props.modelValue?.timer) {
      const realV = clamp(v, 0, Math.floor(props.base.w / 5))
      emit('update:model-value', { ...props.modelValue, timer: { ...props.modelValue.timer, fontGap: realV / props.base.w } })
    }
  }
})
...
const tcolor = computed({
  ...
  set: (v: string) => {
    if (props.modelValue?.timer) {
      emit('update:model-value', { ...props.modelValue, timer: { ...props.modelValue.timer, timeColor: v } })
    }
  }
})
</script>
<template>
  <section class="layout-row">
    ...
    <div class="layout-row-inner">
      <div class="layout-cell">
        <VRadio v-model="clockDisplayType" :value="CLockDisplayType.Digital" label="数字时钟" name="newline" color="primary" />
      </div>
      <div class="layout-cell">
        <VRadio v-model="clockDisplayType" :value="CLockDisplayType.LED" label="LED 时钟" name="newline" color="primary" />
      </div>
    </div>
  </section>
  <section class="layout-row">
    ...
    <div class="layout-row-inner">
      <div class="layout-cell half">
        ...
        <input id="timeWinx" v-model="timeX" type="number" min="0" :max="base.w" step="2">
      </div>
      <div class="layout-cell half">
        <label for="ty">y</label>
        <input id="timeWiny" v-model="timeY" type="number" min="0" :max="base.h" step="2">
      </div>
    </div>
  </section>
  ...
  <template v-else>
    ...
    <section class="layout-row">
      <div class="layout-row-inner">
        <div class="layout-cell">
          <div>颜色</div>
          <color-picker v-model:pureColor="tcolor" format="rgb" shape="square" theme="black" picker-type="chrome" />
        </div>
      </div>
    </section>
    <section class="layout-row">
  ...
  <section class="layout-row">
    ...
    <div class="layout-row-inner">
      <div class="layout-cell">
        <VRadio v-model="time24" :value="24" label="24小时制" name="time24" color="primary" />
      </div>
      <div class="layout-cell">
        <VRadio v-model="time24" :value="12" label="12小时制" name="time24" color="primary" />
      </div>
    </div>
  </section>
  <section class="layout-row">
    ...
    <div class="layout-row-inner">
      <div v-if="clockDisplayType === CLockDisplayType.Digital" class="layout-cell">
        <VRadio v-model="datetype" :value="0" label="YYYY年MM月DD日" name="datetype" color="primary" />
      </div>
      <div class="layout-cell">
        <VRadio v-model="datetype" :value="1" label="YYYY-MM-DD" name="datetype" color="primary" />
      </div>
    </div>
  </section>
  <section class="layout-row">
